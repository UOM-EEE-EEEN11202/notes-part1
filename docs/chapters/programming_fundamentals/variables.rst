...
Values are put into variables by using the equals :python:`=` operator. The variable name goes on the left, and the value to store goes on the right. For example in Python:
...
.. figure:: chess_board.svg.png
  ...
  Illustration of a chess board, drawing from `Wikipedia <https://en.wikipedia.org/wiki/Algebraic_notation_%28chess%29>`_. Re-used under `CC BY-SA license <https://creativecommons.org/licenses/by-sa/3.0/>`_.
...
Tuples
------
A *tuple* groups data together separated by commas :python:`,` and usually inside round brackets :python:`()` . For the example above, the position could thus be
...
The individual *elements* in a tuple can be accessed by using square brackets :python:`[]` and a number as an *address*. These addresses start at 0. So, :python:`piece1_pos_t[0]` contains :python:`"a"` and :python:`piece1_pos_t[1]` contains :python:`3`.
