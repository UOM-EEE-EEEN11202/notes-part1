...
Typically we use *Big O* notation to represent complexity. If :math:`n` is the number of values present in the elements array, the above example would be :math:`O(n)`.

Different code could be :math:`O(n^2)`. Here, doubling the elements from 5 to 10 would result in code which takes 4 times as long to run :math:`(10^2/5^2)`. Code which is :math:`O(n^3)` or even :math:`O(n!)` is possible, and may take a long time to run when you have lots of data! With some thought, lots of algorithms can be written to be :math:`O(n \log n)`, but this might require some effort and not be the first approach you come up with.
